...
public final class AppConfiguration
{
    ...
    static {
        StateControlServlet stateServlet = StateControlServlet.getInstance( );
        final String configFilePath =
                stateServlet.getServletContext().getRealPath("WEB-INF" +
                        "/hazelcast.xml");

        Config hazlecastConfig = null;
        try {
            hazlecastConfig = new FileSystemXmlConfig(configFilePath);
        } catch (FileNotFoundException e) {
            System.err.println("could not read Hazelcast configuration: " +
                    e.getMessage( ));
            e.printStackTrace( );
            System.exit(127);
        }

        HAZELCAST_INSTANCE = Hazelcast.newHazelcastInstance(hazlecastConfig);
